...
Follow these steps to install the integration and run it for the first time against an on-premises ONTAP SVM.

### Step 1 — Install via the One-Command Installer

Run the installer script directly from GitHub. It will detect your Linux distribution, install all system and Python dependencies, clone this repository, create a Python virtual environment, and prompt you for your ONTAP and Veza credentials.
...
/opt/netapp-veza/
├── scripts/
│   ├── netAppShares.py       # Main integration script
│   ├── requirements.txt
│   ├── .env                  # Generated by installer (your credentials)
│   └── venv/                 # Python virtual environment
└── logs/
```

> **Non-interactive / CI mode** — pass all required values as environment variables:
>
> ```bash
...
After the installer finishes, activate the virtual environment before running the script:

```bash
source /opt/netapp-veza/scripts/venv/bin/activate
```

Your shell prompt will change to indicate the environment is active (e.g., `(venv)`).

### Step 3 — Run the Integration Script

Navigate to the scripts directory and execute the sync:
...
| Flag | Value | Notes |
|------|-------|-------|
| `--system-type` | `ontap` | On-premises ONTAP mode |
| `--svm-name` | `TESTCIFSLOW` | Replace with your SVM name |
| `--protocol` | `cifs` | Retrieve CIFS/SMB shares |
| `--folder-depth` | `1` | Discover immediate child folders within each share; increase to go deeper |

Check the output for `✓ Successfully pushed to Veza` to confirm a successful run. Logs are written to `/opt/netapp-veza/logs/`.
...
| `--folder-depth` | No | Integer ≥ 0 | `1` | Maximum folder depth to traverse within each share. `0` = no folders, `1` = immediate child folders only, `2` = subfolders one level deeper, etc. |
...
# Fetch CIFS shares and discover immediate child folders (default depth = 1)
python3 netAppShares.py --system-type ontap --svm-name TESTCIFSLOW --protocol cifs --folder-depth 1

# Discover folders two levels deep
python3 netAppShares.py --system-type ontap --svm-name PROD_SVM --protocol cifs --folder-depth 2

# Skip folder discovery entirely
python3 netAppShares.py --system-type ontap --svm-name PROD_SVM --protocol cifs --folder-depth 0
